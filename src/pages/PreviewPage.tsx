...
export function PreviewPage() {
  ...
  if (!siteData) {
    return (
      <div className="min-h-screen bg-transparent flex flex-col items-center justify-center text-zinc-900 dark:text-white p-6">
        <AlertTriangle className="w-16 h-16 text-yellow-500 mb-6" />
        <h1 className="text-2xl font-bold mb-4 text-center">Nenhum site encontrado na memória</h1>
        <p className="text-zinc-600 dark:text-neutral-400 text-center max-w-md mb-8">
          Por favor, utilize o gerador de IA para criar a estrutura do seu site primeiro.
        </p>
        <button 
          onClick={() => navigate('/generate')}
          className="bg-emerald-600 hover:bg-emerald-500 text-zinc-900 dark:text-white px-8 py-3 rounded-full font-bold transition-all"
        >
          Ir para o Gerador
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      */}
      <div className="fixed top-0 left-0 w-full h-16 bg-transparent border-b border-emerald-500/20 z-[100] flex items-center justify-between px-4 lg:px-8 shadow-2xl">
        ...
        <div className="flex items-center gap-4">
          ...
          {!isSuccess && (
            <button 
              onClick={() => navigate('/generate')}
              className="text-zinc-600 dark:text-neutral-400 hover:text-zinc-900 dark:text-white text-sm hidden sm:block transition-colors"
          ...
          {isSuccess ? (
            <div className="flex items-center gap-3">
              ...
              <a 
                ...
                className="flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#20b958] text-zinc-900 dark:text-white font-bold px-4 sm:px-6 py-2 rounded-lg text-sm transition-all shadow-lg shadow-[#25D366]/30 animate-pulse"
              >
            ...
            <button 
              ...
              disabled={isSubmitting}
              className="flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-zinc-900 dark:text-white font-bold px-6 py-2 rounded-lg text-sm transition-all shadow-lg shadow-emerald-600/20 disabled:opacity-50 min-w-[160px]"
            >
              {isSubmitting ? (
                <>
      ...
      */}
      ...
      <div className="fixed bottom-6 left-6 z-[100] animate-in fade-in slide-in-from-bottom-10 duration-1000 delay-700 hidden sm:block">
        <div className="bg-white rounded-2xl p-6 shadow-2xl border border-neutral-100 max-w-sm ring-1 ring-black/5">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="w-full flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#20b958] text-zinc-900 dark:text-white font-bold px-4 py-3 rounded-xl text-sm transition-all shadow-lg shadow-[#25D366]/30 hover:-translate-y-0.5"
          >
            <MessageCircle size={18} />
      ...
      <div className="pointer-events-none fixed bottom-6 right-6 z-[90] flex flex-col items-end gap-2 opacity-80 scale-90 sm:scale-100 origin-bottom-right">
        <div className="bg-black/60 backdrop-blur-xl px-4 py-2.5 rounded-2xl border border-white/20 flex items-center gap-3 shadow-2xl">
          <Logo showText={true} iconSize={24} textColor="text-zinc-900 dark:text-white" />
          <div className="w-px h-6 bg-white/20"></div>
          <span className="text-zinc-900 dark:text-white font-bold text-xs tracking-wider uppercase">Gerado por IA</span>
        </div>
      </div>
    </div>
  );
